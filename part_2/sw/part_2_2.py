...
# In train dataset, the abstract_embedding column was the string type
# Thus, we changed it as list contains float numbers
for i in range(len(train)):
    x = train['abstract_embedding'][i]
    x = x.strip('][').split(', ')
    x = [float(i) for i in x]
    train['abstract_embedding'][i] = x

# We take the next step to use `claim_embedding` and `abstract_embedding` columns as variable in the following algorithms
...
train[np.arange(768,768+len(train['abstract_embedding'][0]))] = pd.DataFrame(train.abstract_embedding.tolist())
test[np.arange(768,768+len(test['abstract_embedding'][0]))] = pd.DataFrame(test.abstract_embedding.tolist())
dev[np.arange(768,768+len(dev['abstract_embedding'][0]))] = pd.DataFrame(dev.abstract_embedding.tolist())

X_train = train[list(range(0, 1536))]
Y_train = DataFrame(train, columns = ['output'])
X_dev = dev[list(range(0, 1536))]
Y_dev = DataFrame(dev, columns = ['output'])
X_test = test[list(range(0, 1536))]
...
with open('./new_test_set_pred_1.jsonl', 'w') as outfile:
...
with open('./new_test_set_pred_2.jsonl', 'w') as outfile:
